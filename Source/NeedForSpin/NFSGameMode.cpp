    ...
    if(APlayerController* PlayerController = GetWorld()->GetFirstPlayerController()) {
        if(APlayableCharacter* PlayableCharacter = Cast<APlayableCharacter>(PlayerController->GetPawn())) {
            ...
            if(PlayableCharacter->GetPlayerMovementComponent() == nullptr) {
                PlayableCharacter->SetUpPlayerMovementComponent(WalkMovementComponentClass);
            }else if(PlayableCharacter->GetPlayerMovementComponent()->IsA(WalkMovementComponentClass)){
